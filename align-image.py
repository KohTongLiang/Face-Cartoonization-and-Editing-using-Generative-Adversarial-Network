...
def image_align_24(src_file, dst_file, face_landmarks, output_size=256, transform_size=1024, enable_padding=True):
    ...
    # Crop.
    border = max(int(np.rint(qsize * 0.1)), 3) + 2
    crop = (int(np.floor(min(quad[:,0]))), int(np.floor(min(quad[:,1]))), int(np.ceil(max(quad[:,0]))), int(np.ceil(max(quad[:,1]))))
    crop = (max(crop[0] - border, 0), max(crop[1] - border, 0), min(crop[2] + border, img.size[0]), min(crop[3] + border, img.size[1]))
    if crop[2] - crop[0] < img.size[0] or crop[3] - crop[1] < img.size[1]:
        img = img.crop(crop)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', '--src', default='./raw_image', help='directory of raw images')
    parser.add_argument('-d', '--dest', default='./aligned_image', help='directory of raw images')
    parser.add_argument('-ty', '--type', default='anime', help='Which type of images to align')
    ...
    if args.type == 'anime':
        ...
        dataset_dir = os.listdir(args.src)

        k = 0
        for images in dataset_dir:
            print(f'Aligning image {images}')

            input_img_name = images
            img = cv2.imread(f'{args.src}/{input_img_name}')
            faces = face_detector.detectMultiScale(img)

            img = Image.fromarray(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))

            if faces == ():
                # dataset_dir.append(images)
                continue
            ...
            for x_, y_, w_, h_ in faces:
                ...
                # output image
                # img_tmp.save(f'{args.dest}/temp-{str(k).zfill(4)}.png')

            image_align_24(img_tmp, f'{args.dest}/aligned-{str(k).zfill(4)}.png', landmark_array, transform_size=256)
            k += 1
        ...
        for img_name in os.listdir(args.src):
            print(f'Aligning image {img_name}')

            # raw_img_path = os.path.join(args.src, img_name)
            raw_img_path = f'{args.src}/{img_name}'
            for i, face_landmarks in enumerate(landmarks_detector.get_landmarks(raw_img_path), start=1):
                aligned_face_path = os.path.join(args.dest, f'align-{img_name}')
