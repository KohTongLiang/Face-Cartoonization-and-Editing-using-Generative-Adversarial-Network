...
def generate_image(kwargs):
    ...
    trunc = kwargs.get('trunc', True)
    trunc_val = kwargs.get('trunc_val', 0.7)
    n_sample = kwargs.get('n_sample', 1)
    network1 = kwargs.get('source', 'networks/ffhq256.pt')
    target = kwargs.get('target_networks', 'networks/tl_fyp.pt')
    number_of_img = kwargs.get('n_img', 1) #@param {type:"slider", min:0, max:30, step:1}
    number_of_step = kwargs.get('n_step', 1) #@param {type:"slider", min:0, max:10, step:1
    swap = kwargs.get('swap', True) #@param {type:"boolean"}
    swap_layer_num = kwargs.get('swap_num', 2) #@param {type:"slider", min:1, max:6, step:1}
...
def image_editing(kwargs):
    ...
    swap_layer_num = kwargs.get('swap_num', 2) #@param {type:"slider", min:1, max:6, step:1}
    outdir = kwargs.get('outdir', 'results')
    sefa_name = kwargs.get('sefa_name', 'edit_output')
    degree = kwargs.get('degree', 6)
    factor = kwargs.get('factor', 'factor.pt')
    ...
    with torch.no_grad():
        ...
        ndarr = grid.mul(255).add_(0.5).clamp_(0, 255).permute(1, 2, 0).to('cpu', torch.uint8).numpy()
        im = pilimg.fromarray(ndarr)
        im.save(f'./asset/{outdir}/{sefa_name}.png')
        images[0].save(f'./asset/{outdir}/{sefa_name}.gif', save_all=True, append_images=images[1:], loop=0, duration=100)

    print('Complete. Images edited.')

def inference(**kwargs):
    sefa = kwargs.get('sefa', False)

    if sefa:
        image_editing(kwargs)
    else:
